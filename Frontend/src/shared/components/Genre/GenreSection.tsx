...
export const GenreSection: React.FC<GenreSectionProps> = React.memo(
  ({ genre, onMovieSelect }) => {
    ...
    const getFilteredMoviesUseCase = new GetFilteredMoviesUseCase(movieRepository);

    const loadMovies = async () => {
      try {
        const filters = { genreId: genre.genreId };
        const pagedMovies = await getFilteredMoviesUseCase.execute(filters, page, 8);
        setMovies((prev) => {
    ...
    // Manejo del scroll y navegación entre películas
    const handleScroll = () => {
    ...
    useEffect(() => {
		if (isInView) {
		  ...
		  animate(scope.current, { opacity: 0.5, scale: 0.5, rotate: 0 });
		}
	  }, [isInView]);

    return (
      <section className="genre-section relative group" ref={scope}>
        ...
        <div className="relative">
          ...
          <div
            ...
            className="movie-list flex gap-x-12 overflow-x-hidden py-6"
          >
            {movies.map((movie) => (
