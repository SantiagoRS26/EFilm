...
export class MovieDetail {
  ...
  constructor(movieData: MovieDetailDTO) {
    ...
    this.genres = movieData.genres
      ? movieData.genres.map((g) => new Genre(g))
      : [];
    this.keywords = movieData.keywords
      ? movieData.keywords.map((k) => new Keyword(k))
      : [];
    this.comments = movieData.comments
      ? movieData.comments.map((c) => new Comment(c))
      : [];
    this.movieLanguages = movieData.movieLanguages
      ? movieData.movieLanguages.map((ml) => new MovieLanguage(ml))
      : [];
  }
}
